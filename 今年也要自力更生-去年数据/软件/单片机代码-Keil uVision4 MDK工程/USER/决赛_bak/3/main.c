...
//	int16_t time = 180 - Inf.time;
	//状态转换区
	if (Inf.time % 30 == 0 && GameStatus != BEGIN)//30s时间片开始
	{
		GameStatus = BEGIN;
...
/*探坑及后退*/
		...
		if ((!GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_12) || !GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_13))//传感器返回为坑
			&& (mypos[0] > 140 || mypos[0] < 125 || mypos[1] > 200 || mypos[1] < 50)//中场线则不视为坑
			&& (Distance(mypos[0], mypos[1], nearest[0], nearest[1]) < 40 || (mypos[0] > 240 || mypos[0] < 15 || mypos[1] > 240 || mypos[1] < 15)))//离目标较近或在场地边缘
		{
			Back(50);
			MakeNextMove = 3;
