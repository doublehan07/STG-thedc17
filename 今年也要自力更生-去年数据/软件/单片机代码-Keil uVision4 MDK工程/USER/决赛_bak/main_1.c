...
void GoTo(int16_t dis)
{
	...
	{
		SetSpeed(dis * 5 / 6 + 1);SetType(GT);
	}		
}

uint8_t IsAHole(uint8_t x, uint8_t y)
{
	if (((x >= holes[0][0] - 15 && x <= holes[0][0] + 15) || (x >= holes[1][0] - 15 && x <= holes[1][0] + 15))
		&& ((y >= holes[0][1] - 15 && y <= holes[0][1] + 15) || (y >= holes[2][1] - 15 && y <= holes[2][1] + 15)))
		return 1;
	else return 0;
}
...
uint8_t FindTempTar(int16_t enemydir, int16_t tardir)
{
	int16_t disenemy = (int16_t)Distance(mypos[0], mypos[1], enemypos[0], enemypos[1]);
	int16_t escapedis = disenemy * (enemydir - tardir) * 3 / 180;
	if (escapedis > SAFEDIS || escapedis < -SAFEDIS)
		return 0;
	else
	{
		escapedis = escapedis > 0 ? escapedis : -escapedis;
		if (escapedis)
			TempDir = enemydir - (enemydir - tardir) * SAFEDIS / escapedis;
		else
			TempDir = tardir;
		return 1;
	}
...
void GoToTarget()
{
  int16_t mydir, tardir, enemydir;
  int16_t transdir, dis, disenemy;
	uint8_t useTemp;
  Last = Now;
...
/*	if (enemyInf.head_x && enemyInf.head_y)
	{
		enemypos[0] = enemyInf.head_x;
		enemypos[1] = enemyInf.head_y;
	}*/
  tardir = FixedTan(target[0] - mypos[0], target[1] - mypos[1]);
  enemydir = FixedTan(enemypos[0] - mypos[0], enemypos[1] - mypos[1]);
  dis = (int16_t)(Distance(mypos[0], mypos[1], target[0], target[1]) + 0.5f);
  disenemy = (int16_t)(Distance(mypos[0], mypos[1], enemypos[0], enemypos[1]) + 0.5f);
	useTemp = disenemy >= dis ? 0 : FindTempTar(enemydir, tardir);
